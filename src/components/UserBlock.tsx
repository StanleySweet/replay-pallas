        ...
        <article className="mb-[1em] pt-3" style={{ borderTop: "1px solid #C7CCD9" }} >
            <div className="flex items-center space-x-4">
                ...
                <div className="flex-1 min-w-0">
                    <h4>{props.user.id !== 0 ?
                        <Link className="flex items-center space-x-4" to={`/UserDetails/${props.user.id}`}> <b className="flex-1 min-w-0">{props.user.nick}</b><span className={`ml-3 ${color} text-xs font-medium me-2 px-2.5 py-0.5 rounded`}>{EUserRole[props.user.role]}</span></Link> :
